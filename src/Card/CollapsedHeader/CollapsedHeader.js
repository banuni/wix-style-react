...
class CollapsedHeader extends WixComponent {
  ...
  render() {
    ...
    const buttonElement = (
      <div className={styles.button} onClick={this.stopPropagation}>
        <Button
          ...
          height="medium"
          prefixIcon={
            this.state.isCollapsed ? <ArrowDownThin /> : <ArrowUpThin />
          }
    ...
    const toggleElement =
      this.props.toggleStyle === 'switch' ? switchElement : buttonElement;
    const switchHeader = this.state.isCollapsed
      ? headerClassesWithoutDivider
      : headerClasses;
